...
class CombineModel:
    def __init__(self):
        ...
        self.translator = Translator("checkpoints/dictionary_translate/dictionary", "checkpoints/dictionary_translate/data")
...
if __name__ == "__main__":
    model = CombineModel()
    saved_data = load_old("checkpoints/dictionary_translate/data/translated.csv")

    test_vi = [item.rstrip() for item in open("checkpoints/dictionary_translate/data/test.vi", "r", encoding="utf8").readlines()]
    if not os.path.exists("checkpoints/dictionary_translate/data/translated_1.csv"):
        with open("checkpoints/dictionary_translate/data/translated_1.csv", "w", encoding="utf8") as f:
            f.write("len||vi||Dictionary\n")
    for vi_text in tqdm(test_vi):
        vi_text = vi_text.strip()
        if vi_text in saved_data:
            output = [str(saved_data[vi_text]["len"]), vi_text]
            for m_name in ["Dictionary"]:
        ...
        with open("checkpoints/dictionary_translate/data/translated_1.csv", "a", encoding="utf8") as f:
            f.write("||".join(output) + "\n")
